        ...
        >
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              className="flex items-center gap-2 text-white text-sm font-medium px-5 py-2 rounded-xl transition-opacity disabled:opacity-40 disabled:cursor-not-allowed"
      ...
      <AnimatePresence mode="wait">
          ...
          <motion.div key="error" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <div>
              ...
              {(errorMsg.toLowerCase().includes("moment") || errorMsg.toLowerCase().includes("try again")) && (
                <button onClick={() => submit(input.trim())} className="mt-2 text-sm font-medium hover:underline" style={{ color: accentColor }}>
                  Try again
                </button>
              )}
            </div>
        ...
        {data && !isLoading && (
          <motion.div key="result" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                ...
                {data.platform && PLATFORM_META[data.platform] && (() => {
                  ...
                  return (
                    <span className="absolute bottom-2.5 left-2.5 text-xs font-medium px-2 py-0.5 rounded flex items-center gap-1.5 text-white bg-black/70">
                      <PIcon className="w-3 h-3" style={{ color }} />{label}
                    </span>
                  );
                })()}
              </div>
            ...
            <div className="p-5">
              <p className="text-slate-800 font-semibold text-sm leading-snug line-clamp-2 mb-0.5">{data.title}</p>
              <p className="text-xs text-slate-400 mb-5">{data.uploader}</p>

              <AnimatePresence>
                {isDownloading && dlPhase !== "error" && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="mb-4 overflow-hidden">
                    <div className="flex items-center justify-between px-4 py-3 rounded-xl border" style={{ background: `${accentColor}11`, borderColor: `${accentColor}33` }}>
                      <span className="text-sm flex items-center gap-2" style={{ color: accentColor }}>
                        <Loader2 className="w-4 h-4 animate-spin" /> Preparing your download…
                      </span>
                      <button onClick={cancel} className="text-xs text-slate-400 hover:text-slate-700 transition-colors">Dismiss</button>
                    </div>
                    <div className="mt-1 h-0.5 rounded-full bg-slate-100 overflow-hidden">
                      <motion.div className="h-full rounded-full w-1/3" style={{ background: accentColor }}
                        animate={{ x: ["0%", "200%", "0%"] }} transition={{ duration: 1.8, repeat: Infinity, ease: "easeInOut" }} />
                    </div>
                  </motion.div>
                )}
                {dl.id !== null && dlPhase === "error" && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
                    ...
                    <AlertCircle className="w-4 h-4 mt-0.5 shrink-0" />{(dl as any).message}
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              {(() => {
                ...
                return (
                  <div className="space-y-4">
                    {videoFmts.length > 0 && (
                      <section>
                        <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-2">Video</p>
                    ...
                    {audioFmts.length > 0 && (
                      <section>
                        <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-2">Audio</p>
                        <div className="grid grid-cols-2 gap-2">
                          {audioFmts.map((fmt) => {
                            const active = dl.id === fmt.format_id && isDownloading;
                            return (
                              <AudioChip key={fmt.format_id} label={fmt.label} ext={fmt.ext} filesize={fmt.filesize}
                                active={active} disabled={isDownloading} accentColor={accentColor}
        ...
        {!hasResult && (
          <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { delay: 0.15 } }} className="text-center py-12">
            <div className="w-16 h-16 rounded-3xl mx-auto mb-4 flex items-center justify-center text-white" style={{ background: accentColor }}>
...
function FormatChip({ label, sub, active, disabled, accentColor, onClick }: {
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      ...
      style={{ background: active ? `${accentColor}15` : "#f8fafc", border: `1px solid ${active ? `${accentColor}44` : "#e2e8f0"}` }}>
      <div>
...
function AudioChip({ label, ext, filesize, active, disabled, accentColor, onClick }: {
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      ...
      style={{ background: active ? `${accentColor}15` : "#f8fafc", border: `1px solid ${active ? `${accentColor}44` : "#e2e8f0"}` }}>
      <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{ background: active ? accentColor : `${accentColor}22` }}>
