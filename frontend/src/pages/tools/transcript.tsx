...
export default function TranscriptTool() {
  ...
  return (
    <ToolLayout icon={<FileText className="w-4 h-4" />} title="Transcript" subtitle="Generator" accentColor={accentColor}>
      <div>
          ...
          <motion.div key="result" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <div className="px-5 pt-5 pb-4 border-b border-slate-50">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  {result.title && (
                    <p className="text-sm font-semibold text-slate-800 line-clamp-1 mb-1">{result.title}</p>
                  )}
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button onClick={copyText} className="flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors text-slate-600">
                    {copied ? <Check className="w-3.5 h-3.5 text-green-500" /> : <Copy className="w-3.5 h-3.5" />}
                    {copied ? "Copied!" : "Copy"}
                  </button>
                  <button onClick={downloadTxt} className="flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg text-white" style={{ background: accentColor }}>
            ...
            <div className="p-5 max-h-[28rem] overflow-y-auto">
              {showTimestamps ? (
                <div className="space-y-2">
                  {result.entries.map((entry, i) => (
                    <div key={i} className="flex gap-3">
                      <span className="text-[11px] font-mono shrink-0 pt-0.5" style={{ color: accentColor }}>
                ...
                <p className="text-sm text-slate-600 leading-relaxed whitespace-pre-wrap">{result.full_text}</p>
              )}
            </div>
          </motion.div>
        )}
